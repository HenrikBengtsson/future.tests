make_test(.title = "futureAssign() - lazy evaluation", lazy = TRUE, .expr = {
  delayedAssign("a", {
...
make_test(.title = "futureAssign() - potential task name clashes", .expr = {
...
make_test(.title = "futureAssign() - global variables with and without lazy evaluation", lazy = c(FALSE, TRUE), .expr = {
  ## Global variables
  a <- 1
  futureAssign("b", { 2 * a })
  a <- 2
...
make_test(.title = "futureAssign() - lazy evaluation via disposable option", lazy = c(FALSE, TRUE), .expr = {
